...
public abstract class MotorcycleImpl implements Motorcycle {
    ...
    private void setModel(String model) {
        if (null == model || model.trim().isEmpty() || model.length() < MIN_MODEL_LENGTH) {
            throw new IllegalArgumentException(String.format(ValidationMessages.Motorcycle.MODEL_VALIDATION, model, MIN_MODEL_LENGTH));
        }
    ...
    private void setHorsePower(int horsePower) {
        if (horsePower < this.getPowerMin() || horsePower > this.getPowerMax()) {
            throw new IllegalArgumentException(String.format(ValidationMessages.Motorcycle.HORSE_POWER_VALIDATION, horsePower));
        }
